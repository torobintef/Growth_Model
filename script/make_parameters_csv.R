...
for(i in 1:2){
  for(j in 1:2){
    data1 <- read.csv(paste0("result/glmm/FE_",functional[i],"_",leaf[j],".csv"),header = T) %>%
      subset(Row.names == "two_DB" | Row.names == "two_EB") %>%
      mutate(distance = rep(1:50, each = 2),LT = leaf[i],FT = functional[j])
    data2 <- read.csv(paste0("result/glmm/Summary_",functional[i],"_",leaf[j],".csv"),header = T) %>%
      select(sd.two_EB,sd.two_DB)
