...
deriv4 <- derivatives(fit4, select = "s(distance)")
plot(deriv1$distance,deriv1$.derivative,type = "l",col = col1)
lines(deriv1$distance,deriv2$.derivative,type = "l",col = col2)
lines(deriv1$distance,deriv3$.derivative,type = "l",col = col3)
lines(deriv1$distance,deriv4$.derivative,type = "l",col = col4)
...
deriv4$zero_in_ci <- deriv4$.lower_ci <= 0 & deriv4$.upper_ci >= 0
...
saturation_point3
saturation_point4 <- min(deriv4$distance[deriv4$zero_in_ci])
saturation_point4
...
y4 <- approx(d4$distance,pred4,xout = saturation_point4)$y

svg(paste0("figure/distance_",exfname,".svg"),width = 5.7,height = 5,family = "Aerial")
...
lines(d1$distance,pred4,col = col4,lwd = 3)
...
lines(d1$distance,d4$deltaAIC,col = adjustcolor("black",alpha.f = 0.5),lwd = 1.5)
segments(
...
segments(
  x0 = saturation_point4, x1 = saturation_point4,
  y0 = par("usr")[3],y1 = y4,
  lty = 2,lwd = 3,col = adjustcolor(col4, alpha.f = 1)
)
legend("topleft",
       legend = c(
         expression(italic("Q. acuta")~"(DBH<30)"),
         expression(italic("Q. acuta")~"(DBH>=30)"),
         expression(italic("A. amoenum")~"(DBH<30)"),
         expression(italic("A. amoenum")~"(DBH>=30)")
       ),
...
points(saturation_point4,y4,col = col4,cex = 1.5)
box(lwd = 2)
...
mtext("（C）優占種",side = 3,line = 1,cex = 1,family = "BIZ UDPGothic")
